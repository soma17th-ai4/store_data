...
중간에 Supabase statement timeout 등으로 멈췄다면, 마지막 출력의 `processed` 값을 `--skip`에 넣어 이어서 실행합니다. 예를 들어 `processed=4000`까지 성공했다면:

```bash
python store.py --skip 4000
```

`store.py`는 `_id` 오름차순으로 읽기 때문에 `--skip` 기준이 안정적입니다. Supabase upsert 중 timeout이 나면 현재 batch를 자동으로 더 작은 batch로 쪼개 재시도합니다.
